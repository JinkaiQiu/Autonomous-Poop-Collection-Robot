# Want to get the HSV value by calling the kinect2 camera at real time
# need to run the kinect2 bridge to access the image.
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        if image_subscriber.image is not None:
            ...
            # Call find_circles function with current tuning parameters
            keypoints, out_image, tuning_image = find_circles(image_subscriber.image, tuning_params)

            # Display output image with detected circles
